...
// LoginResponse — champs à plat, adapte les noms exacts selon ton swagger
type LoginResponse struct {
	Token    string `json:"token"`
	ID       string `json:"id"`
	Username string `json:"username"`
...
func (c *APIClient) Login(username, password string) error {
	...
	// Stocke le token + les infos utilisateur, tout est dans la même réponse
	c.Token = loginResp.Token
	c.UserID = loginResp.ID
	c.Username = loginResp.Username
	c.Email = loginResp.Email
